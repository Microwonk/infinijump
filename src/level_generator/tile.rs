...
impl Tile {
    pub fn pos(&self) -> (i32, i32) {
        match self {
            Tile::Layered { pos, .. } => *pos,
            Tile::Generic { pos, .. } => *pos,
        }
    }

    pub fn z_index(&self) -> i32 {
        match self {
            Tile::Layered { z_index, .. } => *z_index,
            Tile::Generic { z_index, .. } => *z_index,
        }
    }
}

// TODO: this is just overengineered, should use a struct/enum with data and act on that data seperately for visuals
